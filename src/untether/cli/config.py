...
from ..settings import UntetherSettings, validate_settings_data
...
def _fail_missing_config(path: Path) -> None:
    ...
    if path.exists():
        typer.echo(f"error: invalid untether config at {display}", err=True)
    else:
        typer.echo(f"error: missing untether config at {display}", err=True)
...
def _normalized_value_from_settings(
    settings: UntetherSettings, segments: list[str]
) -> Any:
    node: Any = settings
    for segment in segments:
