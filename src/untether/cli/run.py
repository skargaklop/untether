...
from ..settings import UntetherSettings, load_settings, load_settings_if_exists
...
def _load_settings_optional() -> tuple[UntetherSettings | None, Path | None]:
    try:
...
def _resolve_setup_engine(
    ...
    resolve_plugins_allowlist_fn = cast(
        Callable[[UntetherSettings | None], list[str] | None],
...
def _run_auto_router(
    *,
    ...
    load_settings_fn = cast(
        Callable[[], tuple[UntetherSettings, Path]],
