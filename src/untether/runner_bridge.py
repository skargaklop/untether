...
_DEFAULT_PREAMBLE = (
    "[Untether] The user is on Telegram and sees only your final assistant text, "
    "not tool calls or terminal output. "
    "Use AskUserQuestion when you need clarification so the user receives buttons. "
    "To send a file, write it to `.untether-outbox/`; the user can also request "
    "a project file with `/file get <path>`. "
    "`~/.untether/untether.toml` hot-reloads, so do not restart Untether after "
    "config edits unless a restart-required warning appears. "
    "Always answer the user's request before mentioning hook or cleanup concerns."
)
