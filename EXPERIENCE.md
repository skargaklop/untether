...
The authorised `@notebookassistantbot` poller started as the only Untether process, registered `/menu`, and advertised dynamic `cline` in its startup command list. Desktop Telegram could be restored and inspected, but Orca returned `window_not_focused` for the required keyboard action after its one permitted restore retry; no `/menu` message was sent. The live interaction gate is therefore blocked rather than asserted from a simulated result. The exact keyboard, closed callback mapping, disabled-topic replay, and dynamic Engines filtering remain covered by focused loop and command tests.
