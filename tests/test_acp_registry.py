...
def test_official_npx_distribution_without_matching_launcher_is_not_installed() -> None:
    agent = parse_registry_agents(
        {
            "version": REGISTRY_DOC_VERSION,
            "agents": [
    ...
    other = InstalledLauncher(
        ecosystem="npm",
        package="other-agent",
        version="1.0.0",
        command="C:/Tools/other.cmd",
        metadata_path="C:/npm/node_modules/other-agent/package.json",
    )

    record = discover_installation(
        agent, target="windows-x86_64", cache=None, installed=(other,)
    )
