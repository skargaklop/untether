...
async def test_timed_jsonl_reader_wired_for_omp_and_grok(engine: str) -> None:
    ...
    if engine == "omp":
        ...
        runner = GrokRunner(grok_cmd=sys.executable, extra_args=[], model=None)

    proc = await anyio.open_process(
        [
            sys.executable,
            "-c",
            'import sys; sys.stdout.buffer.write(b\'{"type": "message_start"}\\n\')',
        ]
    )
    try:
        lines = [
        ...
        assert lines == [b'{"type": "message_start"}']
    finally:
        await proc.wait()
